...
const OriginButton = React.forwardRef<HTMLButtonElement, OriginButtonProps>(
  ({ children, className, disabled = false, loading = false, type = "button",
    ...
    React.useEffect(() => {
      ...
      console.warn("OriginButton: provide visible label text or aria-label / aria-labelledby.");
    ...
    const setMergedRef = React.useCallback((node: HTMLButtonElement | null) => {
    ...
    return (
      <motion.button
        ...
        disabled={isDisabled}
        onBlur={(e) => { onBlur?.(e); setIsPressed(false); if (!e.defaultPrevented) setHovered(false); }}
        onClick={onClick}
        onFocus={(e) => { onFocus?.(e); if (isDisabled || e.defaultPrevented) return; if (e.currentTarget.matches(":focus-visible")) { updateOriginFromCenter(); setHovered(true); } }}
        onKeyDown={(e) => { onKeyDown?.(e); if (e.defaultPrevented || isDisabled || e.repeat || (e.key !== " " && e.key !== "Enter")) return; if (e.key === " ") e.preventDefault(); updateOriginFromCenter(); setIsPressed(true); setHovered(true); }}
        onKeyUp={(e) => { onKeyUp?.(e); if (e.key === " " || e.key === "Enter") { setIsPressed(false); if (!e.currentTarget.matches(":focus-visible")) setHovered(false); } }}
        onPointerCancel={(e) => { onPointerCancel?.(e); setIsPressed(false); }}
        onPointerDown={(e) => { onPointerDown?.(e); if (e.defaultPrevented || isDisabled || e.button !== 0) return; updateOriginFromPointer(e); setIsPressed(true); setHovered(true); }}
        onPointerEnter={(e) => { onPointerEnter?.(e); if (isDisabled || e.defaultPrevented) return; updateOriginFromPointer(e); setHovered(true); }}
        onPointerLeave={(e) => { onPointerLeave?.(e); setHovered(false); setIsPressed(false); }}
        onPointerUp={(e) => { onPointerUp?.(e); setIsPressed(false); }}
        ref={setMergedRef}
        type={type}
